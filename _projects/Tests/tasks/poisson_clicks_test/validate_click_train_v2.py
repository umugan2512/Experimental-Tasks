...
# -*- coding: utf-8 -*-
"""
Offline validation of click_train_v2.py -- no Bpod/hardware involved. Simulates many trials per
...
Forked from validate_click_train.py (kept byte-identical/untouched) with only the import point
at click_train_v2 instead of click_train -- everything else reads grid/timing/scheduler values
off the imported module rather than hardcoding them, so it applies unchanged to the new
G05/G12/G22/G38/G65/AOS grid and 2s/0.75s timing.

Run with the pybpod-environment interpreter:
    /c/Users/2P-Behav/.conda/envs/pybpod-environment/python.exe validate_click_train_v2.py
"""
import numpy as np
...
fig.tight_layout()
out_path = 'click_train_validation_v2.png'
