...
# -*- coding: utf-8 -*-
"""
Combined Poisson-clicks + wheel-coupled Gabor-patch test (AOS-only stimulus content, single-valve
reward). Forked from hifi_singleside_test.py (kept byte-identical/untouched) rather than edited in
place, so the already-working audio-only test can't regress -- see repo CLAUDE.md's "Poisson-clicks
/ wheel-turn evidence-accumulation task" section for why new files were used instead. Combines that
script's cue-period/HiFi-playback machinery with gabor_wheel_test.py's wheel-coupled second-monitor
Gabor patch (also byte-identical/untouched, still its own standalone bench test) into a single
choice-phase machine, imported unchanged: `_shared/gabor_display.py`,
`_shared/bpod_trial_helpers.py`'s `TrialRunner`/`was_visited`, `_shared/rotary_setup.py`,
`_shared/hifi_setup.py`. Click generation comes from `poisson_clicks_test/click_train_v2.py` (also
a fork of `click_train.py`, new gamma grid/timing -- see that module's own docstring), not the
original `click_train.py`.

**What's different from hifi_singleside_test.py, and why:**

- Thresholds: choice threshold widened to +/-35deg, hold-steadiness threshold to 4deg -- both IBL
  convention (matching ibllib's own wheel-based-task parameters, not this project's earlier
  +/-30deg/5deg values, which were placeholders carried over from wheel_turn_reward.py). The cue-
  abort threshold stays derived (2.5x the steady threshold -- now 10deg).
- Cue/delay timing shortened (2s cue / 0.75s delay, from click_train_v2.py) and hold-to-init
  duration randomized over a tighter 0.2-0.6s range (was 0.1-0.5s).
- **Lick-gating removed entirely, both places it existed**: `CuePeriod`'s `state_change_conditions`
  no longer listens for `Port1In` at all (only the wheel-abort threshold events do -- the
  `EarlyLick` state is gone), and hold-to-init no longer requires licking to have stopped
  (`wait_for_held_steady(..., require_no_lick=False)`). Lick *events* are still collected and fed
  to the live plots exactly as before -- only the gating/abort behavior is removed, not the
  logging.
- **The choice phase is now a wheel-coupled Gabor patch, not just a go-cue LED.** Structurally, this
  is `gabor_wheel_test.py`'s own choice-phase machine (`WheelGaborPeriod` -> `Reward`/`NoResponse`)
  with `NoReward` renamed/extended to `IncorrectITI` (its own longer `state_timer`, distinct from a
  timeout) and a `Consumption` state after `Reward` (as `hifi_singleside_test.py` already had). Same
  thread-inversion pattern as `gabor_wheel_test.py`: the Bpod call for this machine runs on a
  *background* thread while the *foreground* (main) thread runs the Gabor render loop, because Qt's
  event loop has to run on the thread that owns the `QApplication`; `SystemExit` from a Kill
  received on that background thread is caught and re-raised on the main thread once noticed, same
  as there. `cue_sma` (the audio-only cue period) stays a plain foreground blocking call -- no Qt
  pumping happens during the ~2.75s cue, same trade-off already accepted in `gabor_wheel_test.py`'s
  own design notes.
- **Gabor onset/disappearance are both positive-only jitters, drawn once up front, never chained off
  each other:** `gabor_onset_delay ~ U(0.05, 0.35)`, positive from `WheelGaborPeriod`'s own start
  (LED-on / wheel-turn-becomes-allowed); the patch *freezes* the instant the wheel crosses the
  +/-35deg choice threshold (rather than disappearing immediately); `disappear_delay_s ~
  U(0.4, 0.9)`, positive from that same threshold-crossing instant (not from valve-open), after
  which the patch clears. Reward itself fires immediately on threshold crossing, no jitter -- same
  as every other reward state in this project.
- **Every timing value that gets logged is Bpod-aligned, not a raw render-loop `time.time()`
  reading.** Wheel-threshold crossing is a native Bpod transition (`WheelGaborPeriod` -> `Reward`/
  `IncorrectITI`), so its precise time already sits in `states_durations` after the trial
  (`send_t + visited[state][-1][0]`, the same pattern `CHOICE_MADE`/`TIMED_OUT` already use in
  hifi_singleside_test.py). Every other new event time (`GABOR_ONSET_TIME`, `GABOR_DISAPPEAR_TIME`)
  is computed algebraically from that reference plus the known jitter draw, not from what the render
  loop's own polling happened to observe -- the render loop's live polling is only used to decide
  when to actually draw/undraw pixels in real time. `WHEEL_POS` itself (published continuously by
  `TrialRunner`) already shares this same clock basis (`time.time() - log_python_t0`, the same
  epoch `send_t` is computed from), so it's directly comparable to these new event times at
  analysis time without any conversion.
